import type { PageCount } from "@shared/search/types/pagination";
import type { PaginationConfiguration } from "@shared/search/hooks/pagination";
import { useSearch, type SearchController } from "@shared/search/hooks/search";

import { useAlert } from "@components/feedback/alert/controller";

import type { Bundle } from "@features/bundle/types/bundle";
import { useBundleService } from "@features/bundle/hooks/bundle-service";

export interface BundleSearchConfiguration extends PaginationConfiguration { }

export interface BundleSearchController extends SearchController<Bundle> { }

export function useBundleSearch(
    config?: BundleSearchConfiguration
): BundleSearchController {
    const alert = useAlert();
    const service = useBundleService();

    return useSearch<Bundle>({
        ...config,
        fetchCount: async (): Promise<PageCount> => {
            return service.countAll()
                .catch((error: Error) => {
                    alert.showErrorMessage(error);
                    return 0;
                });
        },
        fetchData: async (pagination): Promise<Bundle[]> => {
            return service.getAll({ pagination })
                .catch((error: Error) => {
                    alert.showErrorMessage(error);
                    return [];
                });
        },
    });
}
